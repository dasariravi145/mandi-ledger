...
object RoyalHeritageTemplateRenderer {

    fun renderRoyalHeritageTemplate(
        html: String,
        ...
        renderedHtml = renderedHtml.replace("{{businessType}}", "Commission Agent") // Default
        ...
        val taglines = if (taglineStr.contains("|")) taglineStr.split("|") else listOf(taglineStr)
        renderedHtml = renderedHtml.replace("{{tagline1}}", taglines.getOrElse(0) { "" }.trim())
        renderedHtml = renderedHtml.replace("{{tagline2}}", taglines.getOrElse(1) { "" }.trim())
        renderedHtml = renderedHtml.replace("{{tagline3}}", taglines.getOrElse(2) { "" }.trim())
        ...
        renderedHtml = replaceImage(renderedHtml, "{{watermarkBase64}}", "{{watermarkHidden}}", profile.stampPath)
        ...
        invoice.products.forEachIndexed { index, product ->
            ...
            rowsHtml.append("<td class='text-center'>KG</td>")
        ...
        // Calculate commission percent if subtotal > 0
