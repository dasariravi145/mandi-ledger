...
object DiamondBusinessEliteTemplateRenderer {

    fun renderDiamondBusinessEliteTemplate(
        html: String,
        ...
        // Logo handling: If logo exists, show it. Otherwise show initial.
        val companyInitial = if (!profile.companyName.isNullOrEmpty()) profile.companyName[0].toString() else "D"
        renderedHtml = renderedHtml.replace("{{companyInitial}}", companyInitial)
        
        if (!profile.logoPath.isNullOrEmpty() && File(profile.logoPath).exists()) {
            renderedHtml = replaceImage(renderedHtml, "{{logoBase64}}", "{{logoHidden}}", profile.logoPath)
            renderedHtml = renderedHtml.replace("{{logoTextHidden}}", "hidden")
        } else {
            renderedHtml = renderedHtml.replace("{{logoHidden}}", "hidden")
            renderedHtml = renderedHtml.replace("{{logoTextHidden}}", "")
        }
        ...
        renderedHtml = renderedHtml.replace("{{customerGstin}}", "") // Placeholder
        renderedHtml = renderedHtml.replace("{{placeOfSupply}}", "") // Placeholder
        renderedHtml = renderedHtml.replace("{{reverseCharge}}", "No")
        ...
        // Amount Section
        renderedHtml = renderedHtml.replace("{{amountInWords}}", Formatter.numberToWords(invoice.grandTotal.toLong()))
        ...
        // Taxes (Default to 0 for now as per project context)
        renderedHtml = renderedHtml.replace("{{taxableAmount}}", Formatter.formatCurrency(invoice.grandTotal))
        renderedHtml = renderedHtml.replace("{{cgst}}", "0.00")
        renderedHtml = renderedHtml.replace("{{sgst}}", "0.00")
        renderedHtml = renderedHtml.replace("{{igst}}", "0.00")
