...
object GkFruitsClassicTemplateRenderer {

    fun renderGkFruitsClassicTemplate(
        html: String,
        ...
        renderedHtml = renderedHtml.replace("{{addressHidden}}", if (profile.address.isBlank()) "hidden" else "")
        
        renderedHtml = renderedHtml.replace("{{mobile}}", profile.mobile)
        renderedHtml = renderedHtml.replace("{{mobileHidden}}", if (profile.mobile.isBlank()) "hidden" else "")
        
        renderedHtml = renderedHtml.replace("{{proprietor}}", profile.proprietor)
        renderedHtml = renderedHtml.replace("{{proprietorHidden}}", if (profile.proprietor.isBlank()) "hidden" else "")
        ...
        invoice.products.forEachIndexed { index, product ->
            ...
            // Show quantity with original unit (Requirement 1)
            val qtyWithUnit = "${Formatter.formatWeight(product.quantity)} ${product.unit}"
            rowsHtml.append("<td class='text-center'>$qtyWithUnit</td>")
